...
export const AboutSectionPortfolio: React.FC<AboutSectionPortfolioProps> = ({
  onContactClick,
}) => {
  ...
  return (
    <section
      id="about"
      className="w-full relative bg-[#fff6e5] text-[#1E1B4B] px-5 sm:px-8 md:px-10 py-24 flex flex-col justify-center overflow-hidden border-b border-[#1E1B4B]/10"
    >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center relative z-10 w-full">
        ...
        <div className="lg:col-span-5 flex flex-col items-start text-left gap-6 pr-0 lg:pr-6">
          ...
          >
            AXI is built by Agile Labs — 27 years of enterprise software behind
            it, a US-patented low-code core, and a track record that includes
            Indian government departments, state data centers, and over
            1,00,000+ users running on the platform today.
          </motion.p>
          ...
          <div className="grid grid-cols-2 gap-4 w-full pt-2">
            {statMetrics.map((m, idx) => (
              <div
                key={idx}
                className="flex flex-col gap-0.5 border-l-2 border-[#00007f] pl-3"
              >
                <AnimatedCounter
                  value={m.value}
                  suffix={m.suffix}
                ...
                {loopTimeline.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-mono font-bold text-[#fc8151] uppercase tracking-wider">
                        {item.tag}
                      </span>
                      <h4 className="text-xs font-black text-[#1E1B4B] uppercase leading-tight">
                        {item.title}
                      </h4>
                      <p className="text-[11px] text-[#373e79] font-normal leading-snug line-clamp-2">
                        {item.desc}
                      </p>
                    </div>
                  </div>
                ...
                {loopGroupA.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <h4 className="text-xs font-black text-[#1E1B4B] uppercase leading-tight mt-0.5">
                        {item.title}
                      </h4>
                      <p className="text-[11px] text-[#373e79] font-normal leading-snug line-clamp-2">
                        {item.desc}
